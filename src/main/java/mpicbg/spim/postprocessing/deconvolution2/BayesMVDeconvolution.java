...
public class BayesMVDeconvolution implements Deconvolver
{
	...
	if(debug){
	    ...
	    for ( int view = 0; view < numViews; ++view )
		{
		    ...
		    final Image<FloatType> data_input = processingData.getImage();
		    final FileSaver inputsaver = new FileSaver(ImageJFunctions.copyToImagePlus( data_input ));
		    inputsaver.saveAsTiffStack(targetdir+inputname+".tif");

		    final Image<FloatType> data_weights = processingData.getWeight();
		    final FileSaver weightssaver = new FileSaver(ImageJFunctions.copyToImagePlus( data_weights ));
		    weightssaver.saveAsTiffStack(targetdir+weightsname+".tif");

		    final Image<FloatType> data_kernel1 = processingData.getKernel1();
		    final FileSaver kernel1saver = new FileSaver(ImageJFunctions.copyToImagePlus( data_kernel1 ));
		    kernel1saver.saveAsTiffStack(targetdir+kernel1name+".tif");

		    final Image<FloatType> data_kernel2 = processingData.getKernel2();
		    final FileSaver kernel2saver = new FileSaver(ImageJFunctions.copyToImagePlus( data_kernel2 ));
		    kernel2saver.saveAsTiffStack(targetdir+kernel2name+".tif");
